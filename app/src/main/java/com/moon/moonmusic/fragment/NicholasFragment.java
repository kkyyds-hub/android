...
public class NicholasFragment extends Fragment {
    ...
    private void initData() {
        // 静态文案来自 Repository，页面只负责展示，方便讲解“数据层”和“界面层”的分工。
    ...
    private void initListener() {
        ...
        // 视频页入口：从专区跳转到 VideoActivity，演示多界面跳转和视频播放功能。
    ...
    private Bitmap downloadBitmap(String urlText) {
        ...
        try {
            ...
            // 图片特效前置知识点：这里先通过 HTTP 拿到图片流，再用 BitmapFactory 解码成 Bitmap。
    ...
    private Location getBestLastKnownLocation(LocationManager locationManager) {
        ...
        // 同时尝试 GPS、网络和被动定位，取时间最新的一条，演示效果比只查一个来源更稳定。
    ...
    private void openDetail() {
        // 详情页使用 WebView 加载本地 H5，是专区中“移动端页面嵌入网页”的演示入口。
